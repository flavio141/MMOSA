...
from models import ModelFinal, VAE, Model3, ModelSurv
...
def create_masking(features_train):
    ...
    for idx, (patient, feature) in enumerate(features_train.items()):
        if feature.shape[0] > 10000:
            ...
            features_padding = torch.full((10000, 1024), -999, dtype=feature.dtype)
...
def train_test_moma(args, train_loader, val_loader, writer, cv, device):
    ...
    #model = VAE(input_size=1536, latent_dims=128, surv_nodes=[128, 1])
    model = ModelSurv(input_size=1024, surv_nodes=[1024, 64, 1], dropout=0.4)
    model.to(device)
    ...
    #criterion = criterions['VAE']
    criterion_cox = criterions['VAECox']
    optimizer = optimizers['AD']

    #scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=10, factor=0.7, threshold=2e-06)
